...
(function(){
  ...
  const kind = document.getElementById('drawingLightboxKind');
  ...
  function openDrawing(opener){
    ...
    const assetKind = opener.dataset.drawingKind || 'ARCHITECTURAL DRAWING';
    const alt = opener.dataset.drawingAlt || label;
    const isDocument = /DOCUMENT|PERMIT|NOTICE/i.test(assetKind);

    image.src = src;
    image.alt = alt;
    if(title) title.textContent = label;
    if(kind) kind.textContent = assetKind;
    lightbox.classList.toggle('is-document', isDocument);
    resetView();
  ...
  function closeDrawing(){
    ...
    lightbox.classList.remove('is-document');
    if(kind) kind.textContent = 'ARCHITECTURAL DRAWING';
    resetView();
...
// 외부 프로젝트 페이지에서 #contact로 들어왔을 때 초기 위치만 보정합니다.
// 사용자가 직접 스크롤을 시작한 뒤에는 폰트/이미지 로딩으로 위치를 다시 강제하지 않습니다.
(function () {
  ...
  window.addEventListener('wheel', cancelAutoAlign, { once: true, passive: true });
  window.addEventListener('touchstart', cancelAutoAlign, { once: true, passive: true });
  window.addEventListener('pointerdown', cancelAutoAlign, { once: true, passive: true });
  window.addEventListener('keydown', cancelAutoAlign, { once: true });

  function alignContact() {
    if (userInteracted || window.location.hash !== '#contact') return;
  ...
  // 초기 페이지 로딩이 아직 끝나지 않은 경우 한 번만 추가 보정합니다.
  if (document.readyState !== 'complete') {
    window.addEventListener('load', realignOnce, { once: true });
  }

  // 폰트 로딩 완료 시점도 사용자가 아직 조작하지 않은 경우에만 한 번 보정합니다.
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(() => {
      if (!userInteracted) realignOnce();
    });
  }
})();
